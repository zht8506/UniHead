...
class RepPointsHead_Uni(AnchorFreeHead):
    """RepPoint head.
    ...
    def _init_layers(self):
        ...
        self.cls_convs.append(
            CIT(self.feat_channels, 2, mlp_ratio=2, reducedim=False,out=True,normlatter=True)
        )
    ...
    def init_weights(self):
        ...
        torch.nn.init.constant_(self.reppoints_cls_out.bias, bias_cls)
    ...
    def forward_single(self, x):
        ...
        cls_feat, pts_feat=decouple_x,decouple_x
        cls_feat=self.pos_ca_cls(cls_feat)
        pts_feat=self.pos_ca_reg(pts_feat)
        cls_feat_=None
        pts_feat_=None


        for cls_conv in self.cls_convs:
            cls_feat_ = cls_conv(cls_feat,pts_feat)
        for reg_conv in self.reg_convs:
            pts_feat_ = reg_conv(pts_feat,cls_feat)

        for cls_conv in self.cls_convs_2:
            cls_feat = cls_conv(cls_feat_,pts_feat_)
        for reg_conv in self.reg_convs_2:
            pts_feat = reg_conv(pts_feat_,cls_feat_)
